...
def erstelle_deck():
    ...
    return [(wert, farbe) for wert in werte for farbe in farben], {wert: index for index, wert in enumerate(werte)}
...
def ist_straight(karten, wert_rang):
...
def ist_straight_flush(karten, wert_rang):
    return ist_flush(karten) and ist_straight(karten, wert_rang)

def ist_royal_flush(karten, wert_rang):
    werte_der_karten = sorted([karte[0] for karte in karten])
    return ist_straight_flush(karten, wert_rang) and werte_der_karten == ['10', 'Bube', 'Dame', 'König', 'Ass']

def poker_simulation(anzahl_spiele, deck, wert_rang):
    ...
    for _ in range(anzahl_spiele):
        ...
        if ist_royal_flush(karten, wert_rang):
            kombinationen_zaehler['Royal Flush'] += 1
        elif ist_straight_flush(karten, wert_rang):
        ...
        elif ist_straight(karten, wert_rang):
...
def main():
    deck, wert_rang = erstelle_deck()
    anzahl_spiele = int(input("Wie viele Spiele sollen simuliert werden? "))
    ergebnisse = poker_simulation(anzahl_spiele, deck, wert_rang)
